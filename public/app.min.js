class BusMapApp {
  ...
  showRouteForVehicle(routeId, vehicleId, color) {
    // Ukloni prethodnu trasu ako postoji
    if (this.activeRouteLayer) {
      this.map.removeLayer(this.activeRouteLayer);
      this.activeRouteLayer = null;
    }
    
    // Koristi shape_id koji je vezan za ovo konkretno vozilo
    ...
    // Ako imamo tačan shape_id od vozila, koristi ga
    if (shapeIdFromVehicle && this.shapesData[shapeIdFromVehicle]) {
      const shapePoints = this.shapesData[shapeIdFromVehicle];
      ...
      if (shapePoints && shapePoints.length > 0) {
        ...
        // UKLONJENO: automatsko fitBounds koje je pravilo zoom
        // this.map.fitBounds(polyline.getBounds(), { padding: [50, 50] });
        
        console.log('Route displayed:', shapeIdFromVehicle, 'with', shapePoints.length, 'points');
        return;
      }
    }
    
    // Fallback: ako nema shape_id od vozila, traži bilo koji za tu rutu
    let matchingShapes = [];
    ...
    const selectedShape = matchingShapes[0];
    const shapePoints = this.shapesData[selectedShape];
    ...
    // UKLONJENO: automatsko fitBounds
    // this.map.fitBounds(polyline.getBounds(), { padding: [50, 50] });
    
    console.log('Route displayed (fallback):', selectedShape, 'with', shapePoints.length, 'points');
  }

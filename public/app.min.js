class BusMapApp {
  ...
  async loadVehicles() {
    ...
    if (data && data.vehicles) {
      ...
      // Ažuriraj Google Sheets
      await this.updateGoogleSheet(data.vehicles, vehicleDestinations);
    }
  }

  async updateGoogleSheet(vehicles, vehicleDestinations) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error('✗ Greška pri ažuriranju Sheets:', errorData);
        return;
      }
      
      const result = await response.json();
      console.log('✓ Google Sheets ažuriran:', {
        novi: result.newVehicles,
        ažurirani: result.updatedVehicles,
        ukupno: result.totalProcessed
      });

      // Ažuriraj i Departures sheet
      try {
        const departuresResponse = await fetch('/api/update-departures-sheet', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (departuresResponse.ok) {
          ...
          console.log('✓ Departures sheet ažuriran:', departuresResult);
        }
      } catch (depErr) {
        console.log('⚠️ Departures skip:', depErr.message);
      }
    } catch (error) {
      console.error('✗ Greška pri ažuriranju Sheets:', error.message);
    }
  }
